...
def phasecode(M):
    ...
    phi_1 = np.pi / M * (k*1000) #+ np.exp(1)/10000
...
if __name__ == '__main__':
    ...
    distance = np.linspace(-0.5*Rmax, 0.5*Rmax, M*N) #M = Ng?
    ...
    pc = fn.PulseCompr(x_win, np.roll(x_win, -22), win = 1, unit='linear')
    ...
    plt.figure()
    plt.plot((distance), fftshift(20*np.log10(abs(pc))),'k*-')
