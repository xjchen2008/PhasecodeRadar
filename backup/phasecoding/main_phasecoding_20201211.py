...
def wavetable(N, win= True, phi = 0):
    ...
    y_cx_pad = np.pad(np.multiply(y_cx_sine1, win),int(N/2),'constant') # for N= 20
    noise = np.random.normal(0, 1e-3, len(y_cx_pad))
    #plt.plot(phase_mod,'o-')
    #plt.show()
    return y_cx_pad # np.multiply(y_cx_sine1, win) #y_cx_sine1 #np.multiply(y_cx_chirp, win) #np.multiply(y_cx_sine3, y_cx_sine3) #y_cx_sine4 #y_cx_pad +noise
...
if __name__ == '__main__':
    ...
    Fp0 = 500e3/a*2#16e3 * a # PRF Related to range resolution and range gate. full phase-coded signal is 1ms duration as FMCW SDR radar
    Fp = M * Fp0
    Tp0 = 1 / Fp0
    Tp = 1 / Fp
