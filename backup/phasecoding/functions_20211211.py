...
def save_file_usrp(y_cx_in,N):
    ...
    y_cxnew = np.around(32767 * y_cx_in/abs(y_cx_in).max())  # np.multiply(y_cx,win)
    y_cxnew = (32767 * y_cx_in / abs(y_cx_in).max())  # np.multiply(y_cx,win)
    yw = np.zeros(2 * N)
    for i in range(0, N):
        yw[2 * i + 1] = np.imag(y_cxnew[i])  # tx signal
        yw[2 * i] = np.real(y_cxnew[i])  # tx signal
    yw1 = np.int16(yw)  # E312 setting --type short; The int data type precision is worse than float, but this is the only format accepted by the SDR RFNoC
...
def plot_freq_db(freq, x, color='b', normalize=False, domain='freq'):
    ...
    if normalize:
        plt.plot(fftshift(freq)/1e6, fftshift(X_log_normalize), '*-', color=color)
    else:
        plt.plot(fftshift(freq)/1e6, fftshift(X_log), '*-', color = color)
    plt.grid(b= True)
