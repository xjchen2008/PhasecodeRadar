...
def wavetable(N, win=True, phi=0):
    ...
    y_cx_pad = np.pad(np.multiply(y_cx_chirp, win), 190*2, 'constant')
...
if __name__ == '__main__':
    ...
    a = 10*40/2
    M = int(1*a)  # int(50 /a)  # tune with Fp0 to increase range gate or range ambiguity
    Fp0 = 500e3*10/a  # 16e3 * a # PRF Related to range resolution and range gate. full phase-coded signal is 1ms duration as FMCW SDR radar
    Fp = M * Fp0
    Tp0 = 1 / Fp0
    Tp = 1 / Fp
    ...
    # fn.plot_freq_db(freq, x_win, normalize=False, domain='time')
    plt.plot(fftshift(freq), fftshift(20 * np.log10(1e-1 + abs(np.fft.fft(x_win_delay, axis=0)))))
    ...
    fn.save_file_usrp(x_win_delay, len(x_win_delay))
    plt.show()
